<template>
	<div class="sellhim">
		<div class="left">
			<div class="main">
				<div class="title">出售{{datas.currencyType}}给他</div>
				<!--单价-->
				<div class="price">
				...
				<div class="content">
					<div class="left_div">
						<!--你想出售多少BIDT?-->
						<div class="tit">你想要出售多少{{datas.currencyType}}？</div>
						<!--左 输入框-->
						<div class="l_num">
							<div class="inp">
								<el-input v-model='l_inp' clearable placeholder="输入你想出售的数量" @input='left_inp'>
							    	<template slot="append">{{datas.currencyType}}</template>
							  	</el-input>
							  	<!--<div class="text">需≥100BIDT</div>-->
							</div>
					...
					<div class="right_div">
						...
						<!--右 输入框-->
						<el-input v-model='r_inp' clearable placeholder="输入你想获得的金额" @input='right_inp'>
					  	...
					  	<div class="text">
					  		...
					  		<!--支付出售-->
					  		<div class="num1">
					  			<div class="num1_l">支付出售：</div>
					  			<div class="num1_r">{{l_inp}} {{datas.currencyType}}</div>
					  		</div>
					  		<!--30分钟-->
					  		<span>请在下单后30分钟内完成支付</span>
					  		<!--出售按钮-->
					  		<div class="send_sell" @click="sell_send">立即出售</div>
					  	</div>
					</div>
				</div>
			</div>
			...
			<div class="know">
				...
				<li>1、下单前，请详细了解对方的交易信息，并确认交易内容无误后，再操作出售。</li>
				<li>2、下单后，请在30分钟内转币至对方指定账户，并在转币完成后在交易订单上点击【我已支付】，逾期系统将自动取消交易。</li>
				<li>3、转币完成后，若30分钟内未收到对方的应付交易资产，可联系TTC-OTC官方客服。</li>
				<li>4、在交易过程中，请务必使用站内聊天系统，避免资料外泄；同时平台外的对话记录将无法作为交易纠纷的依据。</li>
				<li>5、下单后请履行契约精神，恶意抬价或者是反悔，被投诉将冻结账户。</li>
			</div>
		</div>
		<!--右侧留言-->
		<div class="right">
      ...
      if (isNaN(x1)) {
        ...
        // }
				...
				$.ajax({
					...
					success(res){
						...
						if(res.data.list.length===1){
							...
							vm.quota=res.data.list[0].minQuota;
			...
			right_inp(){
				...
				for (var i = 0; i < vm.datas.list.length; i++) {
					if(vm.value_methods===vm.payType[vm.datas.list[i].payType]){
							vm.l_inp=(vm.r_inp/vm.datas.list[i].unitPrice)
					}
				}
			},
			open(val) {
