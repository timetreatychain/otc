<template>
	...
	<div class="Selladvertise">
		...
		<div class="main">
			<div class="title">
				<div class="name">您发布出售 {{datas.currencyType}} 的交易广告<span>{{datas.customer}}</span>已向您下单成功！</div>
				<div class="time">{{datas.createTime}}</div>
			</div>
			<div class="list">
				<div class="list_title">
					<div class="t1 tt1">出售币种</div>
					<div class="t1">出售数量</div>
					<div class="t2">单价</div>
					<div class="t1">收款方式</div>
					<div class="t2">收款金额</div>
				...
				<div class="list_num" v-if="status==='1'||status==='2'">
					待支付出售：{{datas.bidt}} {{datas.currencyType}}
				</div>
				<div class="list_num" v-if="status==='3'||status==='4'">
					已支付出售：{{datas.bidt}} {{datas.currencyType}}
				</div>
				<div class="list_num" v-if="status==='0'||status==='5'">
					<s>待支付出售：{{datas.bidt}} {{datas.currencyType}}</s>
				</div>
			</div>
			<div class="message">
				<div class="message_title">对方收款账号或收币地址请向对方确认</div>
				<!--蛋生支付-->
				<div class="eggzf" v-if="datas.projectType!=0&&datas.eggOrderDetails!=null">
				...
				<!--货币支付-->
				<div class="hbzf" v-if="datas.isEgg!=1">
					<!-- <div class="cnyzf" v-if="payType[datas.type]==='CNY'&&datas.isEgg!=1">
					...
					<!-- <div class="bbzf" v-if="payType[datas.type]==='BTC' &&datas.isEgg!=1" >
						收币地址：{{datas.userAddressVo.btcaddress}}
					</div>
					<div class="bbzf" v-if="payType[datas.type]==='ETH' &&datas.isEgg!=1">
						收币地址：{{datas.userAddressVo.ethaddress}}
					</div>
					<div class="bbzf" v-if="payType[datas.type]==='USDT' &&datas.isEgg!=1">
						收币地址：{{datas.userAddressVo.usdtaddress}}
					</div>
					<div class="bbzf" v-if="payType[datas.type]==='BIDT' &&datas.isEgg!=1">
						收币地址：{{datas.userAddressVo.bidtaddress}}
					</div> -->
					<!-- <div class="bbzf" v-if="payType[datas.type]==='BTC'||payType[datas.type]==='ETH'||payType[datas.type]==='CNY'">
						收币地址：{{datas.userAddressVo.bidtaddress}}
					</div> -->
				</div>
				<div class="hbzf" v-if="datas.isEgg!=1">
					...
					<div class="bbzf" v-if="datas.currencyType==='BTC' &&datas.isEgg!=1" >
						BTC收币地址：{{datas.userAddressVo.btcaddress}}
					</div>
					<div class="bbzf" v-if="datas.currencyType==='ETH' &&datas.isEgg!=1">
						ETH收币地址：{{datas.userAddressVo.ethaddress}}
					</div>
					<div class="bbzf" v-if="datas.currencyType==='USDT' &&datas.isEgg!=1">
						USDT收币地址：{{datas.userAddressVo.usdtaddress}}
					</div>
					<div class="bbzf" v-if="datas.currencyType==='BIDT' &&datas.isEgg!=1">
						BIDT收币地址：{{datas.userAddressVo.bidtaddress}}
					</div>
				</div>
				<div class="message_title">联系对方</div>
				<div class="message_content">
					<div class="messcon_title">您可以在这开始与买家交流了！</div>
				...
				$.ajax({
					...
					success(res){
						...
						if(res.data.orderStatus==='1'){
							vm.left_state='待收款';
							vm.statustext='等待买家向您付款';
						}
						if(res.data.orderStatus==='2'){
							vm.left_state='待支付';
							vm.statustext='买家已付款，请发币';
						}
						if(res.data.orderStatus==='3'){
							vm.left_state='待确认';
							vm.statustext='您已放币，等待买家确认';
						}
