<template>
	...
	<div class="sellSuccess">
		...
		<div class="main">
			<div class="title">
				<div class="name">您向<span>{{datas.customer}}</span>出售{{datas.currencyType}}下单成功！</div>
			...
			<div class="message">
				...
				<div class="hbzf" v-if="datas.projectType!=1">
					<div class="cnyzf" v-if="payType[datas.type] === 'CNY' && datas.currencyType != 'USDT' && datas.currencyType != 'BTC' && datas.currencyType != 'ETH' &&datas.currencyType != 'BIDT'">
					...
					<div class="bbzf" v-if="payType[datas.type]==='CNY'">
						银行卡：{{datas.userAddressVo.bankName}}{{datas.userAddressVo.cnyuserName}}{{datas.userAddressVo.cnyaddress}}
					</div> -->

					<div class="bbzf" v-if="datas.currencyType==='BTC'">
						BTC收币地址：{{datas.userAddressVo.btcaddress}}
					</div>
					<div class="bbzf" v-if="datas.currencyType==='ETH'">
						ETH收币地址：{{datas.userAddressVo.ethaddress}}
					</div>
					<div class="bbzf" v-if="datas.currencyType==='BIDT'">
						BIDT收币地址：{{datas.userAddressVo.bidtaddress}}
					</div>
					<div class="bbzf" v-if="datas.currencyType==='USDT'">
						USDT收币地址：{{datas.userAddressVo.usdtaddress}}
					</div>
					<div class="bbzf" v-if="datas.currencyType==='CNY'">
						银行卡：{{datas.userAddressVo.bankName}}{{datas.userAddressVo.cnyuserName}}{{datas.userAddressVo.cnyaddress}}
					</div>
        ...
        <div class="state1" v-if="status !=4 &&  ordertitme ==0">
				...
				<!--卖家取消后-->
				<!--<div class="state1">
					<div class="state1_text">买家已取消了本次交易，请重新出售别的吧！</div>
					<div class="state1_send" @click="tosell">重新前往出售</div>
				</div>-->
				<!--交易过期-->
...
//			          });          
			...
			state1cancel() {
				let vm=this;
		        this.$confirm('无故随意取消订单将影响您在系统的信用分数。', '您确认不想交易了吗?', {
				...
				$.ajax({
					...
					success(res){
						...
						if(res.data.orderStatus==='1'){
							vm.left_state='待支付';
							vm.statustext='告知买家您是否已支付货币？';
						}
						if(res.data.orderStatus==='2'){
							vm.left_state='待收款';
							vm.statustext='等待买家确认';
						}
